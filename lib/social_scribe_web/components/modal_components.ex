defmodule SocialScribeWeb.ModalComponents do
  ...
  @doc """
  Renders a unified CRM modal wrapper.

  This is a generic modal that works with any CRM provider.
  The overlay color is determined by the `crm_provider` attribute.

  ## Examples

      <.crm_modal id="crm-modal" crm_provider={:hubspot} show on_cancel={JS.patch(~p"/back")}>
        Modal content here
      </.crm_modal>
  """
  attr :id, :string, required: true
  attr :crm_provider, :atom, required: true, values: [:hubspot, :salesforce]
  attr :show, :boolean, default: false
  attr :on_cancel, JS, default: %JS{}
  slot :inner_block, required: true

  def crm_modal(assigns) do
    overlay_class = case assigns.crm_provider do
      :hubspot -> "bg-hubspot-overlay/90"
      :salesforce -> "bg-slate-900/80"
      _ -> "bg-slate-900/80"
    end
